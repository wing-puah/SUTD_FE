import { PencilIcon, TrashIcon, ShoppingCartIcon } from '@heroicons/react/solid';
...
const EditButton = () => (
  <Button variant="primary" onClick={() => alert('Edit btn clicked, populate the form!')}>
...
const Badge = ({ children }) => (
  <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">{children}</span>
);
      ...
      <div className="flex-1 flex md:flex-col justify-between items-start md:items-stretch gap-3 px-2">
        <div className="mt-1 flex-1">
          <div className="flex justify-between items-center gap-3">
            ...
            {props.onlyOne ? <Badge>Only One</Badge> : <div className="text-sm text-gray-500">{props.availableStock} piece available</div>}
          </div>
          <p className="block text-sm font-medium text-gray-900 truncate pointer-events-none">{props.title}</p>
          <p className="block text-sm font-medium text-gray-500 pointer-events-none">{props.description}</p>
        </div>
        <div className="flex flex-col md:flex-row gap-3 py-3">
          {props.onAddToCart ? (
            ...
            <>
              <EditButton />
              <DeleteButton text={isDeleting ? 'DELETING...' : 'DELETE'} onClick={() => setIsDeleting(!isDeleting)} />
            </>
          ) : (
            <Button
              variant="primary"
